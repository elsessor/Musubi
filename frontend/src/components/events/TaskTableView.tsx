...
export function TaskTableView({ tasks, onUpdateStatus }: TaskTableViewProps) {
              ...
              <tr key={task.id} className="transition-colors hover:bg-slate-50/80">
                ...
                <td className="px-4 py-3.5 relative">
                  ...
                  {openDropdownId === task.id && (
                    <div className="absolute left-4 top-full z-20 mt-1 w-36 rounded-xl border border-slate-200 bg-white py-1 shadow-lg ring-1 ring-black/5 animate-in fade-in zoom-in-95 duration-100">
                      {STATUSES.map((st) => (
                        <button
                          key={st}
                          type="button"
