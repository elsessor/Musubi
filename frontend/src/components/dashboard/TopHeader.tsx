...
export function TopHeader({
  ...
  // Safeguard: Fallback to resolved organizationName & academicYear if empty in props
  const organizationName = propOrg || dashboardUser.organizationName;
  const academicYear = propYear || dashboardUser.academicYear || "AY 2025–2026";
  ...
  const subtitle =
    role === "Admin"
      ...
      : [organizationName, role === "Student Leader" || role === "Organization Member" ? academicYear : "", greetingDate]
          .filter(Boolean)
          .join(" · ");
      ...
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0">
          <h1 className="truncate text-xl font-extrabold tracking-[-0.02em] text-slate-900 sm:text-[25px]">
            {role === "Admin"
              ? "Admin Dashboard"
              : `Welcome, ${name}!${role === "Student Leader" ? " 👋" : ""}`}
          </h1>
