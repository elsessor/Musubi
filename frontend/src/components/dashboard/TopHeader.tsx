...
export function TopHeader({
  ...
  useEffect(() => {
    ...
    try {
      const unsub = onSnapshot(
        ...
        (snapshot) => {
          ...
          const liveRole =
            data?.role === "Admin" || data?.role === "Student Leader" || data?.role === "Organization Member"
              ...
              : role;
          setLiveUser({
            name: typeof data?.fullName === "string" && data.fullName.trim() ? data.fullName : name,
            role: liveRole,
        ...
        (err) => {
          // Suppress permission-denied errors gracefully when client Firestore rules disallow direct reads
        }
      );
      return unsub;
    } catch {
  ...
  useEffect(() => {
    if (!liveUser.organizationId) {
      setLiveOrganizationName(organizationName);
      return;
    }
    try {
      const unsub = onSnapshot(
        doc(getFirebaseDb(), "organizations", liveUser.organizationId),
        (snapshot) => {
          const orgName = snapshot.data()?.name;
          setLiveOrganizationName(
            typeof orgName === "string" && orgName.trim() ? orgName : organizationName
          );
        },
        (err) => {
          // Suppress permission-denied errors gracefully
        }
      );
      return unsub;
    } catch {
  ...
  }, [liveUser.organizationId, organizationName]);
  ...
  const subtitle =
    liveUser.role === "Admin"
      ...
      : [
          `${liveOrganizationName || "University Student Council"}${academicYear ? ` · ${academicYear}` : ""}`,
          greetingDate
        ].join(" · ");
