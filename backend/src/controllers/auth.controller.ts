...
import { completeUserOnboarding, createOrganization, getAllMembersForAdmin, getCurrentUser, getMyOrganizationJoinRequest, getOrganizationDirectory, getOrganizationForUser, getOrganizationJoinRequests, getOrganizationManagementDetail, getOrganizationMembers, getOrganizationRequests, getOrganizationsForAdmin, inviteOrganizationMember, joinOrganization, loginWithFirebaseToken, reviewOrganizationJoinRequest, reviewOrganizationRequest, updateMemberAssignmentForAdmin, updateOrganizationMemberByLeader, updateOrganizationForAdmin } from "../services/auth.service.js";
...
export async function updateOrganizationMemberController(request: Request, response: Response, next: NextFunction) { try { const token = getBearerToken(request); const body = request.body as Record<string, unknown>; if (!token) throw new AppError("Firebase ID token is required.", 400); const validRole = body.membershipRole === undefined || body.membershipRole === "leader" || body.membershipRole === "committee_head" || body.membershipRole === "member"; if (!validRole || !(body.position === undefined || typeof body.position === "string") || !(body.committeeId === undefined || body.committeeId === null || typeof body.committeeId === "string")) throw new AppError("Invalid member assignment.", 400); const decoded = await firebaseAuth.verifyIdToken(token); await updateOrganizationMemberByLeader(decoded.uid, request.params.organizationId, request.params.memberId, { membershipRole: body.membershipRole as "leader" | "committee_head" | "member" | undefined, position: body.position as string | undefined, committeeId: body.committeeId as string | null | undefined }); response.status(204).send(); } catch (error) { next(error); } }
export async function inviteOrganizationMemberController(request: Request, response: Response, next: NextFunction) { try { const token = getBearerToken(request); const email = (request.body as Record<string, unknown>).email; if (!token) throw new AppError("Firebase ID token is required.", 400); if (typeof email !== "string" || !email.trim()) throw new AppError("A member email is required.", 400); const decoded = await firebaseAuth.verifyIdToken(token); await inviteOrganizationMember(decoded.uid, request.params.organizationId, email); response.status(204).send(); } catch (error) { next(error); } }
